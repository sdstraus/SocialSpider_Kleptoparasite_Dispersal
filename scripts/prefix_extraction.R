...
######## meta-data for SRA upload ########
## Anelosimus eximius

exi_list <- read.delim("exi_out_filelist.txt", header = FALSE)
class(exi_list)

exi_list <- as.data.frame( gsub(".\\*","",exi_list$V1)) %>% 
  rename(name =`gsub(".\\\\*", "", exi_list$V1)`)

exi_list$filename.1<- paste(exi_list$name, ".1.fq.gz", sep = "")
exi_list$filename.2 <- paste(exi_list$name, ".2.fq.gz", sep = "")

write.csv(exi_list, "exi_names.csv", row.names = FALSE)


## Faiditus sp 1


f1_list <- read.delim("f1_out_filelist.txt", header = FALSE)
class(f1_list)

f1_list <- as.data.frame(gsub(".\\*","",f1_list$V1)) %>% 
  rename(name =`gsub(".\\\\*", "", f1_list$V1)`)

f1_list$filename.1<- paste(f1_list$name, ".1.fq.gz", sep = "")
f1_list$filename.2 <- paste(f1_list$name, ".2.fq.gz", sep = "")

write.csv(f1_list, "f1_names.csv", row.names = FALSE)

gi.f2 <- readRDS("../rds/gi.f2.rds")

# Faiditus sp. 2
# need to create list for Faiditus sp. 2
f2_list <- (indNames(gi.f2))
f2_list <- f2_list %>% 
  as.data.frame() %>% 
  rename(., name = `.`) %>% 
